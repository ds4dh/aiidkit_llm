...
from scripts.script_utils import (
    calibrate_array_pair, calibrate_dataframe_pair, paired_bootstrap_pr_auc_pvalue,
)
...
parser.add_argument(
    ...
    help="Select boundary constraint tuning mode (global, window_specific).",
)
parser.add_argument(
    "--target_recall",
    type=int,
...
def apply_fdr_and_export_tables(split_results: dict, analysis_dir: Path, split: str, horizon_str: str):
    ...
    for window_name, record in split_results.items():
        for m in CLASSIC_MODELS:
            ...
            p_pr_str = record.get(f"  Threshold-free PR-AUC p-value ({disp})", "NaN")
            try:
                p_pr = float(p_pr_str)
                if not np.isnan(p_pr):
                    p_entries.append({'window': window_name, 'model': disp, 'test': 'PR-AUC', 'p_val': p_pr})
            except ValueError:
                pass

            p_mcn_str = record.get(f"  McNemar p-value ({disp})", "NaN")
            try:
                p_mcn = float(p_mcn_str)
                if not np.isnan(p_mcn):
                    p_entries.append({'window': window_name, 'model': disp, 'test': 'McNemar', 'p_val': p_mcn})
            except ValueError:
                pass
    ...
    if p_entries:
        ...
        _, q_vals, _, _ = multipletests(raw_pvals, alpha=0.05, method='fdr_bh')
        for idx, item in enumerate(p_entries):
            q_map[(item['window'], item['model'], item['test'])] = q_vals[idx]
    ...
    for window_name, record in split_results.items():
        ...
        for metric_prefix, arrow in METRIC_MAPPING:
            ...
            for model in ["logistic_regression", "random_forest", "xgboost", "Transformer"]:
                disp = MODEL_DISPLAY_MAP[model]
                metric_key = f"  {disp}_{head_lbl}"
                main_win_record[metric_key] = record.get(f"  {metric_prefix} ({arrow}) ({disp})", "")
        ...
        for m in CLASSIC_MODELS:
            ...
            p_pr_raw = float(record.get(f"  Threshold-free PR-AUC p-value ({disp})", "NaN"))
            ...
            p_mcn_raw = float(record.get(f"  McNemar p-value ({disp})", "NaN"))
    ...
    for metric_prefix, arrow in METRIC_MAPPING:
        ...
        for model in ["logistic_regression", "random_forest", "xgboost", "Transformer"]:
            disp = MODEL_DISPLAY_MAP[model]
            main_rows_order.append(f"  {disp}_{head_lbl}")
            main_row_clean_map[f"  {disp}_{head_lbl}"] = f"  {disp}"

    main_rows_order.append("PR-AUC bootstrap statistical winner and p-value")
    main_row_clean_map["PR-AUC bootstrap statistical winner and p-value"] = "PR-AUC bootstrap statistical winner and p-value"
    for m in CLASSIC_MODELS:
        disp = MODEL_DISPLAY_MAP[m]
        main_rows_order.append(f"  TF vs {disp} (PR-AUC)")
        main_row_clean_map[f"  TF vs {disp} (PR-AUC)"] = f"  TF vs {disp}"

    main_rows_order.append("McNemar statistical winner and p-value")
    main_row_clean_map["McNemar statistical winner and p-value"] = "McNemar statistical winner and p-value"
    for m in CLASSIC_MODELS:
        disp = MODEL_DISPLAY_MAP[m]
        main_rows_order.append(f"  TF vs {disp} (McNemar)")
        main_row_clean_map[f"  TF vs {disp} (McNemar)"] = f"  TF vs {disp}"
    ...
    for window_name, win_dict in main_table_data.items():
        ...
        for metric_prefix, arrow in METRIC_MAPPING:
            ...
            for model in ["logistic_regression", "random_forest", "xgboost", "Transformer"]:
                disp = MODEL_DISPLAY_MAP[model]
                col_data[f"  {disp}_{head_lbl}"] = win_dict[f"  {disp}_{head_lbl}"]

        col_data["PR-AUC bootstrap statistical winner and p-value"] = ""
        for m in CLASSIC_MODELS:
            disp = MODEL_DISPLAY_MAP[m]
            col_data[f"  TF vs {disp} (PR-AUC)"] = win_dict[f"  TF vs {disp} (PR-AUC)"]

        col_data["McNemar statistical winner and p-value"] = ""
        for m in CLASSIC_MODELS:
            disp = MODEL_DISPLAY_MAP[m]
            col_data[f"  TF vs {disp} (McNemar)"] = win_dict[f"  TF vs {disp} (McNemar)"]

        formatted_main_data[window_name] = col_data

    df_main = pd.DataFrame(formatted_main_data).reindex(main_rows_order)
    df_main.index = df_main.index.map(main_row_clean_map)
    df_main.index.name = "Evaluation metric"
    df_main = df_main.fillna("")

    main_out_path = analysis_dir / f"{split}_{horizon_str}_main_summary_report.csv"
    df_main.to_csv(main_out_path)
...
def process_split_strategy(task: str, split: str, horizon, analysis_dir: Path) -> dict:
    ...
    report_out_path = analysis_dir / f"{split}_{horizon_str}_head_to_head_report.csv"
    ...
    for w_idx, (window_name, (low, high)) in enumerate(CLINICAL_WINDOWS.items()):
        ...
        for m in CLASSIC_MODELS:
            ...
            try:
                ...
                winner_mcnemar = "Tie" if p_val_mcnemar > 0.05 else ("TF" if b > c else disp)
            except Exception:
                p_val_mcnemar = np.nan
                winner_mcnemar = "NaN"
               
            p_val_pr_auc = paired_bootstrap_pr_auc_pvalue(
                y_true=y_true,
                p_tf=sub_test_df["y_prob_Transformer"].values,
                p_base=sub_test_df[f"y_prob_{m}"].values,
                n_bootstraps=N_BOOTSTRAP
            )
           
            pr_auc_tf = average_precision_score(y_true, sub_test_df["y_prob_Transformer"].values) if len(np.unique(y_true)) >= 2 else 0.0
            pr_auc_base = average_precision_score(y_true, sub_test_df[f"y_prob_{m}"].values) if len(np.unique(y_true)) >= 2 else 0.0
           
            if pd.isna(p_val_pr_auc):
                winner_pr_auc = "NaN"
            else:
                winner_pr_auc = "Tie" if p_val_pr_auc > 0.05 else ("TF" if pr_auc_tf > pr_auc_base else disp)
               
            window_record[f"TF vs {disp}"] = ""
            window_record[f"  Threshold-free PR-AUC p-value ({disp})"] = f"{p_val_pr_auc:.3e}" if not pd.isna(p_val_pr_auc) else "NaN"
            window_record[f"  PR-AUC winner ({disp})"] = winner_pr_auc
            window_record[f"  Discordant pairs (TF correct) ({disp})"] = b
            window_record[f"  Discordant pairs ({disp} correct) ({disp})"] = c
            window_record[f"  McNemar p-value ({disp})"] = f"{p_val_mcnemar:.3e}" if not pd.isna(p_val_mcnemar) else "NaN"
            window_record[f"  McNemar winner ({disp})"] = winner_mcnemar
    ...
    print(f"\n>>> MAIN SUMMARY REPORT (FDR Q-VALUE CORRECTED): STRATEGY SPLIT [{split.upper()}] | HORIZON: {str(horizon).upper()} <<<")
...
def render_matrix_visualization_grids(plotted_data_cache: dict, analysis_dir: Path, horizon_suffix: str):
    ...
    fig_pr.tight_layout()
    fig_pr.savefig(analysis_dir / f"matrix_pr_comparison_curves_{horizon_suffix}.png", dpi=200, bbox_inches='tight')
    plt.close(fig_pr)
...
def process_evaluation_pipeline(task: str, horizon):
    ...
    task_horizon_dir.mkdir(parents=True, exist_ok=True)
   
    plotted_data_cache = {}
   
    for split in SPLIT_TYPES:
        window_cache = process_split_strategy(task, split, horizon, task_horizon_dir)
        if window_cache is not None:
            plotted_data_cache[split] = window_cache
           
    if plotted_data_cache:
        render_matrix_visualization_grids(plotted_data_cache, task_horizon_dir, horizon_str)
